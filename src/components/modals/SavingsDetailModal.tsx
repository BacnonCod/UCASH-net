...
import { CreditCard, PlusCircle, Landmark, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
const SavingsDetailModal: React.FC<SavingsDetailModalProps> = ({ open, onOpenChange }) => {
  const accounts = [
    {
      id: 1,
      ...
      growth: 5.2,
      transactions: [
        { date: 'Jul 15, 2025', description: 'Deposit', amount: 5000, type: 'credit' },
        { date: 'Jul 10, 2025', description: 'Interest', amount: 750.25, type: 'credit' },
        { date: 'Jun 30, 2025', description: 'Transfer to Checking', amount: 2000, type: 'debit' },
      ]
    },
    {
      id: 2,
      ...
      growth: 4.8,
      transactions: [
        { date: 'Jul 12, 2025', description: 'Deposit', amount: 1000, type: 'credit' },
        { date: 'Jul 01, 2025', description: 'Interest', amount: 324.50, type: 'credit' },
      ]
    },
    {
      id: 3,
      ...
      growth: 6.5,
      transactions: [
        { date: 'Jul 15, 2025', description: 'Contribution', amount: 2000, type: 'credit' },
        { date: 'Jul 01, 2025', description: 'Dividend', amount: 830.24, type: 'credit' },
      ]
    }
  ];
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <Tabs defaultValue={`account-${accounts[0].id}`} className="flex-1 overflow-hidden flex flex-col">
          ...
          {accounts.map(account => (
            <TabsContent key={account.id} value={`account-${account.id}`} className="flex-1 overflow-auto mt-4">
                          ...
                          <td className="py-3 px-4 text-sm text-right flex justify-end items-center">
                            {transaction.type === 'credit' ? (
                              <>
                                ...
                                <span className="text-green-600">+${transaction.amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                              </>
                            ) : (
                              <>
                                <ArrowDownRight size={14} className="text-red-600 mr-1" />
                                <span className="text-red-600">-${transaction.amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                              </>
                            )}
                          </td>
                        </tr>
