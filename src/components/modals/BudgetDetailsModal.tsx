...
const BudgetDetailsModal: React.FC<BudgetDetailsModalProps> = ({ open, onOpenChange }) => {
  ...
  const handleAllocationChange = (category: keyof typeof allocations, value: number[]) => {
    ...
    if (total > 100) {
      ...
      const otherCategories = Object.keys(newAllocations).filter(key => key !== category) as Array<keyof typeof allocations>;
      
      const totalOther = otherCategories.reduce((sum, key) => sum + newAllocations[key], 0);
      
      otherCategories.forEach(key => {
        const proportion = newAllocations[key] / totalOther;
        newAllocations[key] = Math.max(0, Math.round(newAllocations[key] - (excess * proportion)));
      });
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
              ...
              <div key={category} className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <div className="text-sm text-gray-500">
                    <span className="font-medium">${calculateAmount(percentage)}</span>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} className="bg-ucash-dark">Save Budget</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
